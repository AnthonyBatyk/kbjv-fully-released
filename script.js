document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderCalculatorLog(){
    calcLog.innerHTML="";if(!calculatorItems.length){calcLog.innerHTML='<div style="padding:10px 0;">Історія порожня.</div>';return;}
    calculatorItems.forEach((item,index)=>{
      const row=document.createElement("div");row.className="log-item calc-log-item";row.draggable=true;row.dataset.index=String(index);
      const handle=document.createElement("span");handle.className="calc-drag-handle";handle.textContent="☰";handle.title="Перетягнути";
      ...
      const actions=document.createElement("div");actions.className="calc-log-actions";actions.append(remove,handle);
      row.addEventListener("dragstart",e=>{draggedCalcIndex=index;row.classList.add("calc-dragging");e.dataTransfer.effectAllowed="move";});
      row.addEventListener("dragend",()=>{draggedCalcIndex=null;row.classList.remove("calc-dragging");calcLog.querySelectorAll(".calc-drag-over").forEach(r=>r.classList.remove("calc-drag-over"));});
      row.addEventListener("dragover",e=>{e.preventDefault();if(draggedCalcIndex!==null&&draggedCalcIndex!==index)row.classList.add("calc-drag-over");});
      row.addEventListener("dragleave",()=>row.classList.remove("calc-drag-over"));
      row.addEventListener("drop",e=>{e.preventDefault();row.classList.remove("calc-drag-over");if(draggedCalcIndex===null||draggedCalcIndex===index)return;const [moved]=calculatorItems.splice(draggedCalcIndex,1);calculatorItems.splice(index,0,moved);saveCalculatorLocal();renderCalculatorLog();updateTotals();logAction("Порядок записів у калькуляторі змінено.");});

      let touchFromIndex=null;
      handle.addEventListener("pointerdown",e=>{
        if(e.pointerType==="mouse")return;
        e.preventDefault();
        touchFromIndex=Number(row.dataset.index);
        row.classList.add("calc-dragging");
        try{handle.setPointerCapture(e.pointerId);}catch{}
      });
      handle.addEventListener("pointermove",e=>{
        if(touchFromIndex===null)return;
        e.preventDefault();
        calcLog.querySelectorAll(".calc-drag-over").forEach(r=>r.classList.remove("calc-drag-over"));
        const target=document.elementFromPoint(e.clientX,e.clientY)?.closest(".calc-log-item");
        if(target&&target!==row)target.classList.add("calc-drag-over");
      });
      const finishTouchDrag=e=>{
        if(touchFromIndex===null)return;
        e.preventDefault();
        const target=document.elementFromPoint(e.clientX,e.clientY)?.closest(".calc-log-item");
        const to=target?Number(target.dataset.index):touchFromIndex;
        row.classList.remove("calc-dragging");
        calcLog.querySelectorAll(".calc-drag-over").forEach(r=>r.classList.remove("calc-drag-over"));
        if(Number.isInteger(to)&&to!==touchFromIndex){
          const [moved]=calculatorItems.splice(touchFromIndex,1);
          calculatorItems.splice(to,0,moved);
          saveCalculatorLocal();renderCalculatorLog();updateTotals();logAction("Порядок записів у калькуляторі змінено.");
        }
        touchFromIndex=null;
      };
      handle.addEventListener("pointerup",finishTouchDrag);
      handle.addEventListener("pointercancel",()=>{touchFromIndex=null;row.classList.remove("calc-dragging");calcLog.querySelectorAll(".calc-drag-over").forEach(r=>r.classList.remove("calc-drag-over"));});
